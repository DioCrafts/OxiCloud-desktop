...
use crate::domain::entities::{AuthSession, SyncConfig, SyncItem, SyncStatus};
use crate::domain::ports::{StorageError, StoragePort, StorageResult, SyncHistoryRecord};
...
impl StoragePort for SqliteStorage {
    ...
    async fn delete_items_under_path(&self, path: &str) -> StorageResult<u32> {
        ...
        let count = conn
            .execute("DELETE FROM sync_items WHERE path LIKE ?", params![pattern])
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
    ...
    async fn get_total_size(&self) -> StorageResult<u64> {
        ...
        let size: i64 = conn
            .query_row("SELECT COALESCE(SUM(size), 0) FROM sync_items", [], |row| {
                row.get(0)
            })
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
    ...
    async fn load_session(&self) -> StorageResult<Option<AuthSession>> {
        ...
        match result {
            Ok((
                ...
                let server_info: crate::domain::entities::ServerInfo =
                    serde_json::from_str(&server_info_json)
                        .map_err(|e| StorageError::SerializationError(e.to_string()))?;
    ...
    async fn save_sync_folders(&self, folder_ids: &[String]) -> StorageResult<()> {
        ...
        for id in folder_ids {
            conn.execute(
                "INSERT INTO sync_folders (folder_id) VALUES (?)",
                params![id],
            )
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
        }
    ...
    async fn load_sync_folders(&self) -> StorageResult<Vec<String>> {
        ...
        let mut stmt = conn
            .prepare("SELECT folder_id FROM sync_folders")
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
