...
//! This module defines all functions and types that can be called from Dart.

use std::sync::Arc;
use tokio::sync::RwLock;
use flutter_rust_bridge::frb;

use crate::domain::entities::{SyncItem, SyncStatus, SyncDirection, SyncConfig, AuthCredentials, ServerInfo};
...
pub async fn initialize(config: SyncConfig) -> Result<(), String> {
    tracing::info!("Initializing sync engine with config: {:?}", config);
    ...
    // Create WebDAV client (will be configured after auth)
...
impl From<ConflictResolutionChoice> for crate::domain::entities::ConflictResolution {
    fn from(choice: ConflictResolutionChoice) -> Self {
        match choice {
            ConflictResolutionChoice::KeepLocal => Self::KeepLocal,
            ConflictResolutionChoice::KeepRemote => Self::KeepRemote,
            ConflictResolutionChoice::KeepBoth => Self::KeepBoth,
            ConflictResolutionChoice::Skip => Self::Skip,
        }
    }
}

/// Helper to get the global engine
